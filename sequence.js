...
const keyToSquare = (key) => {
  ...
  if (!cached || cached.getBoundingClientRect().width === 0) {
    ...
    cachedKToS = Object.fromEntries(numpadInOrder.map((num, idx) => [`Numpad${num}`, squares[idx]]));
...
window.addEventListener('keydown', e => {
  if (e.code === 'Numpad0') {
    simulateClick(Array.from(document.getElementsByTagName('button')).find(el => el.textContent === 'Start'));
    simulateClick(Array.from(document.getElementsByTagName('button')).find(el => el.textContent === 'Try again'));
  } else {
